...
const ANNOTATION_METADATA_KEYS: &[&[u8]] = &[
    b"T",
    b"M",
    b"Contents",
    b"RC",
...
const REVIEW_ANNOTATION_SUBTYPES: &[&[u8]] = &[
    b"Text",
    b"FreeText",
    b"Line",
    b"Square",
...
fn annotation_subtype(dict: &Dictionary) -> Option<&[u8]> {
    dict.get(b"Subtype").and_then(Object::as_name).ok()
}

fn is_review_annotation_dict(dict: &Dictionary) -> bool {
    annotation_subtype(dict)
        .map(|subtype| REVIEW_ANNOTATION_SUBTYPES.contains(&subtype))
        .unwrap_or(false)
}
...
fn collect_annotation_metadata(
    id: ObjectId,
    dict: &Dictionary,
    entries: &mut Vec<MetadataEntry>,
    total_bytes: &mut usize,
) {
    if !is_review_annotation_dict(dict) {
        return;
    }
    ...
    for key in ANNOTATION_METADATA_KEYS {
        ...
        *total_bytes += value.len();
        entries.push(MetadataEntry {
            ...
            name: format!("Object {} {} /{}", id.0, id.1, String::from_utf8_lossy(key)),
            value,
        });
    }
}
...
mod tests {
    ...
    fn doc_with_annotation_metadata() -> Vec<u8> {
        let mut doc = Document::with_version("1.4");
        let catalog_id = doc.new_object_id();
        let pages_id = doc.new_object_id();
        let page_id = doc.new_object_id();
        let annotation_id = doc.new_object_id();
        ...
        doc.objects.insert(
            ...
            dictionary! {
                ...
                "T" => Object::string_literal("Alice Reviewer"),
                "M" => Object::string_literal("D:20260530101010-07'00'"),
                "Contents" => Object::string_literal("Private annotation text"),
                "RC" => Object::string_literal("<body>Rich private annotation</body>"),
                "Subj" => Object::string_literal("Private annotation subject"),
                "CreationDate" => Object::string_literal("D:20260530090000-07'00'"),
                "NM" => Object::string_literal("annotation-secret-id"),
                "Popup" => popup_id,
            }
            .into(),
        );
        doc.objects.insert(
            ...
            dictionary! {
                ...
                "Contents" => Object::string_literal("Popup private note"),
                "M" => Object::string_literal("D:20260530101111-07'00'"),
            }
            .into(),
        );
        doc.objects.insert(
            ...
            dictionary! {
                "Type" => "Annot",
                "Subtype" => "Widget",
                "Rect" => vec![30.into(), 30.into(), 80.into(), 45.into()],
                "T" => Object::string_literal("Preserved form field name"),
            }
            .into(),
        );
        doc.objects.insert(
            ...
            dictionary! {
                ...
                "Parent" => pages_id,
                "MediaBox" => vec![0.into(), 0.into(), 100.into(), 100.into()],
                "Annots" => vec![annotation_id.into(), popup_id.into(), widget_id.into()],
            }
            .into(),
        );
        doc.objects.insert(
    ...
    #[test]
    fn test_remove_metadata_strips_pdf_annotation_author_dates_and_contents() {
        let input = doc_with_annotation_metadata();
        let info = extract_metadata(&input);
        assert!(info
            .metadata_found
            .iter()
            .any(|entry| entry.category == "Annotation" && entry.value.contains("Alice Reviewer")));
        assert!(info
            .metadata_found
            .iter()
            .any(|entry| entry.category == "Annotation"
                && entry.value.contains("Private annotation text")));
        assert!(!info
            .metadata_found
            .iter()
            .any(|entry| entry.category == "Annotation" && entry.value.starts_with("Ref(")));
        ...
        for secret in [
            ...
            "Rich private annotation",
            "Private annotation subject",
            "annotation-secret-id",
            "Popup private note",
        ] {
            assert!(
                !cleaned_text.contains(secret),
        ...
        assert!(cleaned_text.contains("Preserved form field name"));
